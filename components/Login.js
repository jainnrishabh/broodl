...
export default function Login() {
  ...
  return (
    <div className='flex flex-col flex-center justify-center items-center gap-4'>
      ...
      <p className='text-center'>{!isRegister ? 'Already have an account ?':'Dont have an account'} <button onClick={()=>setIsRegister(!isRegister)} className='text-indigo-600'>{isRegister ? 'Sign In' : 'Sign up'}</button></p>
    </div>
  )
}
